...
# Global variables for dependency tracking
deps: list['Dependency'] = []
deps_per_file: dict[str, list['Dependency']] = {}
deps_collected: dict[str, bool] = {}
rpaths_per_file: dict[str, list[str]] = {}
rpath_to_fullpath: dict[str, str] = {}
...
def collect_dependencies(filename: str) -> None:
    """Collect dependencies for a given file."""
    if filename in deps_collected:
        return

    collect_rpaths(filename)
    lines = _collect_dependency_lines(filename)
    ...
    for line in lines:
        ...
        add_dependency(dep_path, filename)

    deps_collected[filename] = True


def _collect_dependency_lines(filename: str) -> list[str]:
...
def collect_rpaths(filename: str) -> None:
    ...
    while pos < len(lc_lines):
        ...
        if read_rpath:
            ...
            if filename not in rpaths_per_file:
                rpaths_per_file[filename] = []
            rpaths_per_file[filename].append(rpath)
            read_rpath = False
            continue
...
def add_dependency(path: str, filename: str) -> None:
    ...
    deps_in_file = deps_per_file.get(filename, [])
    ...
    if not in_deps_per_file:
        deps_per_file[filename] = deps_per_file.get(filename, []) + [dep]


def collect_sub_dependencies() -> None:
    ...
    while True:
        ...
        for dep in deps[:dep_amount]:
            ...
            collect_dependencies(original_path)

        if len(deps) == dep_amount:
            break  # no more dependencies were added on this iteration, stop searching


def done_with_deps_go() -> None:
    ...
    if Settings.bundle_libs_enabled():
        ...
        for dep in reversed(deps):
            ...
            dep.copy_yourself()
            change_lib_paths_on_file(dep.get_install_path())
            fix_rpaths_on_file(dep.get_original_path(), dep.get_install_path())
            adhoc_codesign(dep.get_install_path())
    ...
    for i in range(Settings.file_to_fix_amount() - 1, -1, -1):
        ...
        try:
            ...
            pass
        change_lib_paths_on_file(file_to_fix)
        fix_rpaths_on_file(file_to_fix, file_to_fix)
        adhoc_codesign(file_to_fix)


def create_dest_dir() -> None:
...
def change_lib_paths_on_file(file_to_fix: str) -> None:
    """Change library paths in a file."""
    if file_to_fix not in deps_collected:
        print("    ", end="")
        collect_dependencies(file_to_fix)
        print()

    print(f"  * Fixing dependencies on {file_to_fix}")
    deps_in_file = deps_per_file.get(file_to_fix, [])
    for dep in deps_in_file:
        dep.fix_file_that_depends_on_me(file_to_fix)


def fix_rpaths_on_file(original_file: str, file_to_fix: str) -> None:
    """Fix rpaths in a file."""
    rpaths_to_fix = rpaths_per_file.get(original_file, [])
...
def adhoc_codesign(file: str) -> None:
...
def main():
    ...
    for i in range(Settings.file_to_fix_amount()):
        collect_dependencies(Settings.file_to_fix(i))

    collect_sub_dependencies()
    done_with_deps_go()
